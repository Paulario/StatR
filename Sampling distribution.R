...
# Set up the sample size and the mumber of replicates before hand
  n = 500
  times = 1200000
...
# Suppose we've got a sample and its mean is 135. Calculate p-value that mean <=135
...
(140-summary$mean)/summary$sd

# Calculated based on the given sampling distribution
sample_distribution %>% 
  mutate(prop = ifelse(value <= summary$mean+summary$sd & value >= summary$mean-summary$sd, 1,0)) %>% 
